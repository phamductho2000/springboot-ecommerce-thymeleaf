...
public interface ProductRepository extends JpaRepository<ProductEntity, String>, JpaSpecificationExecutor<ProductEntity> {
    ...
    @Query("SELECT ae.id, ae.name, ae.code, ave.id, ave.value " +
            ...
            "(SELECT ave.id FROM ave.productEntities avpe WHERE avpe.id = ?1) ORDER BY ae.sort")
    List<Object[]> getAttrAndValueByProductId(String productId);
    ...
    List<ProductEntity> findAllBySlugSeo(String slug);
}
